$(function(){
	...
	APP.Entities.Pais = {
		Mostrar : function(){
			APP.View.load('pais', 'pais/form/frm-pais').success(function(_html){
		            ...
		            buttons: [{
		                ...
		                action: function(dialog) {
		                    ...
		                    if(APP.validateForm('.form')){
			                    APP.Entities.Pais.Guardar({
			                    	'descripcion':$('#txtDescripcion').val()
			                    }).success(function(res){
			                    	$button.stopSpin();
			                    	$('.message-msg-area').notification({message:'Guardado correctamente', msgtype:'success', closable:true});
			                    	$('#tblPais').find('tbody').append('<tr data-id="'+res.id+'" class="animated fadeInLeft"><td>'+res.id+'</td><td>'+$('#txtDescripcion').val()+'</td></tr>');
			                    	$('#tblPais').parent().scrollTop($('#tblPais').parent()[0].scrollHeight);
			                    	$('#txtDescripcion').val('');
			                    });
		                    }else{
		                    	$button.stopSpin();
		                ...
		                action: function(dialog) {
		                    var $button = this;
		                    $button.spin();
		                    if($('#tblPais').find('.selectedRow').length == 0){
		                    ...
		                    if(APP.validateForm('.form')){
								APP.Entities.Pais.Actualizar({
									id:$('#tblPais').find('.selectedRow').data('id'),
									descripcion:$('#txtDescripcion').val()
								}).success(function(){
									$button.stopSpin();
			                    	$('.message-msg-area').notification({message:'se actualizó correctamente', msgtype:'success', closable:true});
									$($('#tblPais .selectedRow td')[1]).text($('#txtDescripcion').val());
								});
		                    }else{
								$button.stopSpin();
			                    $('.message-msg-area').notification({message:'algunos campos tienen errores', msgtype:'error', closable:true});	
		                ...
		                action:function(dialog){
					        BootstrapDialog.show({
					            ...
					            buttons: [{
					                ...
					                cssClass: 'btn-primary',
					                action: function(dlg){
					                   var http =  APP.Entities.Pais.Borrar({
					                    	id:$('#tblPais').find('.selectedRow').data('id')
					                    });

					                   http.complete(function(data){
					                   		$('#tblPais').find('.selectedRow').addClass('animated fadeOutLeft');
					                   		$('#tblPais').find('.selectedRow').remove();
											dlg.close();
				...
				dialog.setTitle('Nuevo Pais');
				dialog.open();
			});

		},

		listaDePaises : function(){
			return $.ajax({
				type:'POST',
				url:BASE_PATH + 'pais/',
				dataType: 'JSON'
		...
		Actualizar:function(data){
			return $.ajax({
				type:'POST',
				url:BASE_PATH +'pais/actualizarPais',
				data:data,
				dataType:'JSON'
			});
		},
		...
		RellenarTabla:function(tbl){
			APP.Entities.Pais.listaDePaises().success(function(res){
